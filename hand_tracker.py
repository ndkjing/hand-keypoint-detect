...
class HandTracker():
    ...
    def __call__(self, img,return_track,detect_hand=True,detect_keypoints=True):
        img_pad, img_norm, pad = self.preprocess_img(img)

        if detect_hand:
            return_track = {}
            hand_detect_start_time = time.time()
            source, keypoints = self.detect_hand(img_norm)
            # print('hand detect time spend',time.time()-hand_detect_start_time)
            return_track['source'] = source
            return_track['keypoints'] = keypoints
        else:
            keypoints = return_track['keypoints']
            source = return_track['source']

        if source is None:  # 未检测到
        ...
        return kp_orig, box_orig,return_track